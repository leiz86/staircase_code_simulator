...
int DataManager::init(const char* opts) {
	...
	ccParams = ComponentCodeNS::Params(15, 7, 2, 1); //(511, 484, 3, 1);	// testing only
	if(!ccParams.isValid) {
	...
	scParams = StaircaseCodeNS::Params(7, 3, 1); //(255, 7, 8);	// testing only
	if(!scParams.isValid) {
	...
	simParams = Params(0.2, 0.2, 0.01, 10, 100);
